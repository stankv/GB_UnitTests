...
def test_find_average():
    assert Tasks.find_average([10, 20, 30, 40, 50]) == 30, "Результат не 30 !"
    assert Tasks.find_average([]) == 0, "Тест не сработал в случае пустого массива!"
    assert Tasks.find_average([5]) == 5, "Тест не сработал в случае одного значения!"

# Задание 2
def test_type_error():
    with pytest.raises(TypeError):
        Tasks.find_average(12)
